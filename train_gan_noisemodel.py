...
writer= SummaryWriter(log_dir='./checkpointunderstar')
import time
...
def main():
    ...
    parser.add_argument('--noiselist', default='shot_read_uniform',
...
def mse_to_psnr(mse, max: float = 1.0):
    return 20 * math.log10(max) - 10 * torch.log10(mse)

class PSNRLoss(nn.Module):
    """PSNR Loss assuming input is an image or a batch of images"""
    ...
    def forward(self, input , target) :
        mse = torch.mean(self._mse_loss(input, target), dim=[-1, -2, -3])
        psnr = torch.mean(mse_to_psnr(mse, self._max))
        return psnr
...
def train(gpu, args):
    ...
    for epoch in range(0,num_epochs):
        for i, sample in enumerate(train_loader):
            ...
            if i % n_critic == 0:
                ...
                if args.generator_loss == 'lpips':
                    g_lpips = gen_loss(fake_imgs, real_noisy)
            ...
            if total_steps % 1000 ==0:
                psnr_func = PSNRLoss().to(fake_imgs.device)
                writer.add_scalar("train/PSNR", psnr_func(fake_imgs, real_noisy).item(), total_steps)
                # print(f"PSNR: {psnr_func(fake_imgs, real_noisy).item()}")
                writer.add_scalar("train/G_loss",g_loss.item(),total_steps)
                writer.add_scalar("train/lpips_loss",g_lpips.item(),total_steps)
        ...
        print('KLD', kld_val)
        if total_steps % 1000 ==0:
            writer.add_scalar('train/kld',kld_val,total_steps)
        ...
        if epoch % 5 == 0:
            ...
            for i, sample in enumerate(test_loader):
                ...
                with torch.no_grad():
                    ...
                    kld_val = gh.cal_kld(gen1, real1)
                    tot_kld += kld_val
                writer.add_scalar('test/kld', tot_kld, epoch)
